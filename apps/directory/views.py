...
class EnterpriseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Enterprise.objects.select_related("owner").annotate(
    ...
    def me(self, request):
        ...
        if enterprise is None:
            return Response({"detail": "Aucune entreprise trouvée."}, status=404)
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    search_fields = [
        ...
        "enterprise__name",
        "category__name",
        "category__sub_sector__name",
    ]
    ordering_fields = ["created_at", "updated_at", "indicative_price", "name"]

    def get_queryset(self):
        return Product.objects.select_related(
            "enterprise", "category", "category__sub_sector"
    ...
    def perform_create(self, serializer):
        ...
        if enterprise is None:
            raise ValidationError(
                "Vous devez configurer votre entreprise avant d'ajouter un produit."
            )
...
class SearchView(APIView):
    ...
    def get(self, request):
        ...
        enterprises = Enterprise.objects.select_related("owner").annotate(
            product_count=Count("products")
        )
        ...
        if sector:
            ...
            enterprises = enterprises.filter(
                products__category__sub_sector_id=sector
            ).distinct()
        if category:
        ...
        return Response(
            {
                "products": ProductSerializer(
                    products[:20], many=True, context={"request": request}
                ).data,
                "enterprises": EnterpriseSerializer(
                    enterprises[:20], many=True, context={"request": request}
                ).data,
            }
        )
